...
ksa_require_native_touchid() {
  ...
  printf '%s\n' "$ssh_keygen_help" | grep -q -- '-K' || \
    ksa_die "Apple ssh-keygen does not support resident-key download (-K)"
...
ksa_recover_touchid_handle() {
  recovery_dir="$1"
  install -d -m 0700 "$recovery_dir"
  ksa_log "Touch ID can be requested while OpenSSH obtains the local key handle."
  (
    cd "$recovery_dir"
    KEYCHAIN_CERTIFICATES="$ctk_hash" \
      "$KSA_CTK_SSH_KEYGEN" -K -w "$KSA_CTK_PROVIDER" -N ''
  ) || ksa_die "OpenSSH could not obtain the key handle for: $ctk_label"

  set -- "$recovery_dir"/id_*_sk_rk
  [ "$#" -eq 1 ] && [ -f "$1" ] || \
    ksa_die "OpenSSH did not return exactly one key handle for: $ctk_label"
  recovered_key="$1"
  recovered_pub="${recovered_key}.pub"
  [ -f "$recovered_pub" ] || ksa_die "OpenSSH did not return a public key for: $ctk_label"
  grep -Eq '^sk-ecdsa-sha2-nistp256@openssh.com ' "$recovered_pub" || \
    ksa_die "CryptoTokenKit identity did not produce the required P-256 OpenSSH key"
}
...
ksa_validate_touchid_manifest() {
  [ "$(ksa_manifest_field schema_version "$manifest_path")" = 1 ] || \
...
ksa_ensure_touchid_profile() {
  ...
  if [ -f "$manifest_path" ]; then
    ...
    [ ! -e "$key_path" ] && [ ! -e "$pub_path" ] || \
      ksa_die "approval key files exist without trusted profile metadata: $profile_dir"
    if [ -n "$ctk_hash" ]; then
      ksa_die "CryptoTokenKit identity exists without Klokast profile metadata: $ctk_label"
    fi
  ...
  if [ ! -f "$key_path" ] || [ ! -f "$pub_path" ]; then
    [ ! -e "$key_path" ] && [ ! -e "$pub_path" ] || \
      ksa_die "approval key handle is incomplete: $profile_dir"
    ksa_recover_touchid_handle "$work_dir/recovered-$purpose"
    key_fingerprint="$(ksa_key_fingerprint "$recovered_pub")"
    if [ -f "$manifest_path" ]; then
      [ "$key_fingerprint" = "$(ksa_manifest_field key_fingerprint "$manifest_path")" ] || \
        ksa_die "recovered approval key does not match the recorded fingerprint"
    fi
    install -m 0600 "$recovered_key" "$key_path"
...
ksa_load_touchid_profile() {
  ...
  ksa_validate_touchid_manifest
  [ -f "$key_path" ] && [ -f "$pub_path" ] || \
    ksa_die "Touch ID approval key handle is incomplete: $profile_dir"
  chmod 0600 "$key_path" "$pub_path"
