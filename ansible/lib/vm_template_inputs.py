...
def stage_job_files(root, job_files):
    ...
    for relative, source in extra.items():
        if (not isinstance(relative, str) or
                ...
                not regular(source, 1024 * 1024)):
            raise UpdateError("bootstrap job file has an unsafe path or source")
        target = root / relative
        if any(parent.is_symlink() for parent in target.parents if parent != root.parent):
            raise UpdateError("bootstrap job file parent is a symlink")
        if target.exists() or target.is_symlink():
            raise UpdateError("bootstrap job file overlaps package or job content")
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(source, target)
        target.chmod(0o600)


def bootstrap(directory, output, guest_job, *, expected_profile="shared-alpine-v1", job_files=None):
    ...
    with tempfile.TemporaryDirectory(prefix="unpack-", dir=output) as temporary:
        ...
        stage_job_files(root, job_files)
