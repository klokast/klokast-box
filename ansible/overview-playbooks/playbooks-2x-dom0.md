...
- `dom0-tailscale`: install `tailscale` and `tailscale-openrc`, restart Tailscale after package changes so the daemon matches the CLI, enable and start the service, ensure Tailscale accepts Tailnet DNS and repairs overwritten `/etc/resolv.conf`, align the advertised Tailscale hostname with `node_hostname`, and ensure `tailscale_persist_paths` are tracked by `lbu`.
