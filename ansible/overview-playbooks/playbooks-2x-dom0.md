...
- `dom0-apk-policy`: pin APK repositories to the running Alpine release branch, install the complete runtime and recovery package set, replace checksum constraints and other drift with the exact `dom0_world_packages` list, reconcile installed packages to versions available from that branch, remove maintenance-only packages and unused dependencies, install the APK transaction guard, and remove the RAM-only unlock.
