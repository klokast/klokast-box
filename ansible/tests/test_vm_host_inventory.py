...
class HostInventory(unittest.TestCase):
    ...
    def test_tailscale_resolver_receipt_binds_status_without_exporting_suffix(self):
        status = {'BackendState': 'Running', 'MagicDNSSuffix': 'tail6c4921.ts.net'}
        expected = (b'# resolv.conf(5) file generated by tailscale\n'
                    b'# For more info, see https://tailscale.com/s/resolvconf-overwrite\n'
                    b'# DO NOT EDIT THIS FILE BY HAND -- CHANGES WILL BE OVERWRITTEN\n'
                    b'\nnameserver 100.100.100.100\nsearch tail6c4921.ts.net\n')
        with patch.object(self.m, 'stable_regular_bytes', return_value=expected):
            receipt = self.m.tailscale_resolver(status)
        self.assertEqual(receipt['expected_sha256'], receipt['observed_sha256'])
        self.assertNotIn('tail6c4921.ts.net', json.dumps(receipt))
        with patch.object(self.m, 'stable_regular_bytes', return_value=expected + b'extra'):
            changed = self.m.tailscale_resolver(status)
