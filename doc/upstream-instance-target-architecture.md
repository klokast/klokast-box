...
| Registry source adoption | `implemented` | Engine `e05a713` and the exact private candidate are active. Unsigned preparation found a pending-action vocabulary mismatch and refused before router verification or source publication. Promote the corrected validator, install matching tools, and refresh baselines and evidence before signed live acceptance. Runbook 51 records the attempt and checks. |
