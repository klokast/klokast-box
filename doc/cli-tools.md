...
| `publish-private-instance` | laptop | Accepts the exact seed or a human-edited instance document checked by the sealed controller, asks the human to approve the displayed diff, commits with the human Git identity, pushes with the human GitHub credential, and verifies private `main`. |
