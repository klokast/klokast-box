...
```sh
ansible/bin/platform-image-build --app music --box k001 build
ansible/bin/platform-image-build --app music --box k001 load
...
```sh
klokast-dev/bin/kk music upload --from ~/Documents/music --to k002
```
...
- Default for the `k002` SMSL DAC: `--soundcard hw:CARD=AUDIO,DEV=0`.
